...
class NAEOnlinePredictor:
    def __init__(self, model_path, model_params, training_params, prediction_params, active_range_x=[0, 10000], active_range_y=[0, 10000], active_range_z=[0.1, 10000], swap_y_z=False):
        # Load model
        self.nae = NAE(**model_params, **training_params, device=DEVICE)
        self.nae = NAE.load_model(model_path)
        ...
        self.mocap_sub = rospy.Subscriber('/mocap', PoseStamped, self.mocap_callback)
        # Publish predicted trajectory
        self.predicted_traj_pub = rospy.Publisher('/predicted_traj', PointArray, queue_size=10)
        self.impact_point_pub = rospy.Publisher('NAE/impact_point', Point, queue_size=10)
    ...
    def mocap_callback(self, msg:PoseStamped):
        ...
        # enable_get_mocap_data = self.enable_get_mocap_data_lock.acquire(blocking=False)
        # if not enable_get_mocap_data:
        #     rospy.logwarn(name + 'Disable getting mocap data')
        #     return
        print(name + 'Getting mocap data ...')
    ...
    def online_predict(self):
        while not rospy.is_shutdown():
            ...
            self.printer.print_green(name + 'Press enter to start predicting', background=False)
            ...
            while not rospy.is_shutdown():
                ...
                while not impacted:
                    ...
                    if self.pred_seq is not None:
                        ...
                        input('TODO: check input_data: ', input_data.shape)
                    # add batch dimension to input_data
                    input_data = np.expand_dims(input_data, axis=0)
                    # predict
                    self.pred_seq = self.nae.predict(input_data, evaluation=True).cpu().numpy()
    ...
    def publish_prediction(self, pred_seq, impact_point=True, pred_traj=True):
        if impact_point:
            impact_point = Point()
            impact_point.x = pred_seq[0]
            impact_point.y = pred_seq[1]
            impact_point.z = pred_seq[2]
...
def main():
    ...
    default_model = 'training_scripts/models/my_dataset_models/frisbee-input-15pred-50/run_NAE3L_26-10-2024_04-21-39/@epochs1020_data2_batchsize128_seq15_pred50_timemin34-01'
    ...
    prediction_params = {
        'auto_agressive_len': 2     # number of future prediction steps that are used as input for the next prediction step
    }
    ...
    nae_online_predictor = NAEOnlinePredictor(model_path, model_params, training_params, prediction_params, active_range_x, active_range_y, active_range_z, swap_y_z)
    rospy.spin()
